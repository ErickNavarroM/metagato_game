...
void Menu(Colores &Tema, int dec)
{
    ...
    std::ifstream file;
    char c;
    file.open("color.txt", std::ifstream::in);
    if(file.is_open()){
        c = file.get();
        file.close();
        MoverCursor(2, 3);
...
void Juego(Colores tema, Nombres nombre)
{
    Tablero tablero;
    for(int k = 0; k < 9; ++k){
        tablero.cuadrante[k].lim_izq = POSX_ORIGEN + 6*(k%3) + 1;
        tablero.cuadrante[k].lim_der = tablero.cuadrante[k].lim_izq + 4;
        tablero.cuadrante[k].lim_sup = POSY_ORIGEN + 6*(k/3) + 1;
        tablero.cuadrante[k].lim_inf = tablero.cuadrante[k].lim_sup + 4;
    }

    ImprimirTablero(POSX_ORIGEN,POSY_ORIGEN);
    ...
    while(true){
        ...
        //Dependiendo de d�nde puso el jugador anterior, depender�
        //si la seleccion se limitar� al cuadrante o a todo el tablero.
        ...
        //Una vez el jugador escoja, se imprime el s�mbolo en pantalla.
        ImprimirSimbolo(tema, tablero); //El �ndice de la casilla equivale al indice del proximo cuadrante.

        //Se comprueba si con este �ltimo movimiento un jugador gan� alg�n cuadrante.
        ...
        //Terminado el proceso de selecci�n y comprobaci�n del
...
void Creditos(int tecla)
{
    do {
        ...
        std::cout << "CR" << char(144) << "DITOS:" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-22);
        std::cout << "Programa presentado como proyecto final de Programaci\242n de Computadoras de la" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-21);
        std::cout << "Licenciatura en Ciencias de la Computaci\242n en la Universidad de Sonora." << std::endl;
        MoverCursor(4, ALTO_PANTALLA-20);
        std::cout << "Maestra: Irene Rodriguez Castillo." << std::endl;
        MoverCursor(4, ALTO_PANTALLA-19);
        std::cout << "Equipo 2." << std::endl;
        MoverCursor((ANCHO_PANTALLA-26)/2, ALTO_PANTALLA-16);
        std::cout << "EQUIPO DE DESARROLLADORES:" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-14);
        std::cout << "Galaviz Cuen Oscar Eduardo" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-13);
        std::cout << "Navarro Mel\202ndrez Erick Joel" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-12);
        std::cout << "Ochoa Galv\240n Daniel Rub\202n" << std::endl;
        MoverCursor(4, ALTO_PANTALLA-11);
        std::cout << "Ruiz Gonz\240lez Kevin David" << std::endl;
...
void RulesPg3(int x, int y)
{
    ...
    ImprimirRejillaSimple(21, 25);
    MoverCursor(25, 29);
    EstablecerColor(COLORFONDO, ROJOCLARO);
    std::cout << 'X';
    ...
    EstablecerColor(AMARILLOCLARO, COLORFONDO);
    ImprimirRejillaSimple((x/2 + 21), 33);
    EstablecerColor(COLORFONDO, COLORLETRA);
